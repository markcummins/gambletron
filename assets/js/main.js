...
//	});
	...
	function render_page(json_file_table, json_file_fixtures){
		...
		$.ajax({
			...
			render_league_table(data, 'simple');
			get_win_avg_table(data);
			window.league_table = data;
		});
		...
		$.ajax({
			...
			get_upcoming_fixtures(data);
			window.league_fixtures = data;
		});
	}
...
//				var l1 = '<h2>'+ value['homeTeamName'] +'</h2><div class="progress"><div class="progress-bar w-lbl" style="width: '+ pred['w'] +'%;"><span>Home Win ('+ pred['w'] +'%)</span></div></div>';
//				var l2 = '<h2>VS</h2><div class="progress"><div class="progress-bar d-lbl" style="width: '+ pred['d'] +'%;"><span>Draw ('+ pred['d'] +'%)</span></div></div>';
//				var l3 = '<h2>'+ value['awayTeamName'] +'</h2><div class="progress"><div class="progress-bar l-lbl" style="width: '+ pred['l'] +'%;"><span>Away Win ('+ pred['l'] +'%)</span></div></div>';
				
				var l1 = '<h2>'+ value['homeTeamName'] +'</h2><span class="label w-lbl">'+ value['homeTeamName'] + ' Win ('+ pred['w'] +'%)</span>&nbsp;';
				var l2 = '<h2>VS</h2><span class="label d-lbl">Draw ('+ pred['w'] +'%)</span>&nbsp;';
				var l3 = '<h2>'+ value['awayTeamName'] +'</h2><span class="label l-lbl">'+value['awayTeamName'] + ' Win ('+ pred['w'] +'%)</span>&nbsp;';
				var cf = '';
	...
	function get_team(team_url, elem_id){
		...
		$.ajax({
			...
			$(elem_id).hide().html(res).fadeIn('slow');
		}); 
